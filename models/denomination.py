...
class Denomination(Database):
    def __init__(self, currency: Currency, value: float, denomination_id: int | None = None) -> None:
    ...
    def exists(self) -> bool:
        if self.denomination_id:
            sql: str = "SELECT EXISTS(SELECT 1 FROM denominations WHERE denomination_id = ?) AS denomination_exists"
    ...
    def _update(self) -> None:
        sql: str = "UPDATE denominations SET currency = ?, value = ? WHERE denomination_id = ?"
    ...
    def load_by_denomination_id(cls, denomination_id: int) -> "Denomination" | None:
        ...
        if result:
            ...
            row = result[0]
            return cls(Currency.load_by_currency_id(result["currency"]), row["value"], denomination_id = row["denomination_id"])
        return None
    ...
    def get_all_denominations(cls) -> list["Denomination" | None]:
        ...
        denominations = list()
        for row in result:
            denominations.append(cls(Currency.load_by_currency_id(result["currency"]), row["value"], denomination_id = row["denomination_id"]))
