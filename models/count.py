...
class Count(Database):
    ...
    def exists(self) -> bool:
        if self.count_id:
            sql: str = (
                "SELECT EXISTS(SELECT 1 FROM counts WHERE count_id = ?) AS count_exists"
            )
    ...
    def _update(self) -> None:
        sql: str = (
            "UPDATE counts SET user = ?, date = ?, currency = ? WHERE count_id = ?"
        )
        params: tuple = (
            self.user.user_id,
            self.date.isoformat(" "),
    ...
    def load_by_count_id(cls, count_id: int) -> "Count" | None:
        ...
        if result:
            ...
            row = result[0]
            return cls(
                User.load_by_user_id(row["user"]),
                Currency.load_by_currency_id(row["currency_id"]),
